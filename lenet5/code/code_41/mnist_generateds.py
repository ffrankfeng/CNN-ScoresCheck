...
image_train_path = config.image_train_path_41
label_train_path = config.label_train_path_41
tfRecord_train = config.tfRecord_train_41
image_test_path = config.image_test_path_41
label_test_path = config.label_test_path_41
tfRecord_test = config.tfRecord_test_41
data_path = config.data_path_41
resize_height = 28
resize_width = 28
...
def read_tfRecord(tfRecord_path):
    ...
    features = tf.parse_single_example(serialized_example,  # 一个标量字符串张量
                                       features={  # 一个字典映射功能键FixedLenFeature或VarLenFeature的值
                                           'label': tf.FixedLenFeature([41], tf.int64),
...
def get_tfrecord(num, isTrain=True):
    ...
    img_batch, label_batch = tf.train.shuffle_batch([img, label],  # 待乱序处理的列表中的样本
                                                    ...
                                                    num_threads=8,
                                                    capacity=2000,  # 队列中元素最大数量
                                                    min_after_dequeue=300)  # 出队后队列中最小数量元素，用于确保元素的混合级别

...
image_train_path = './mnist_data_jpg/mnist_train_png_5000/'
label_train_path = './mnist_data_jpg/mnist_train_png_5000.txt'
tfRecord_train = './data/100/mnist_train.tfrecords'
image_test_path = './mnist_data_jpg/mnist_test_png_5000/'
label_test_path = './mnist_data_jpg/mnist_test_png_5000.txt'
tfRecord_test = './data/100/mnist_test.tfrecords'
data_path = './data/100'
resize_height = 28
resize_width = 28
NUMSIZE=101
...
def read_tfRecord(tfRecord_path):
    ...
    features = tf.parse_single_example(serialized_example,  # 一个标量字符串张量
                                       features={  # 一个字典映射功能键FixedLenFeature或VarLenFeature的值
                                           'label': tf.FixedLenFeature([NUMSIZE], tf.int64),
...
def get_tfrecord(num, isTrain=True):
    ...
    img_batch, label_batch = tf.train.shuffle_batch([img, label],  # 待乱序处理的列表中的样本
                                                    ...
                                                    num_threads=2,
                                                    capacity=1000,  # 队列中元素最大数量
                                                    min_after_dequeue=100)  # 出队后队列中最小数量元素，用于确保元素的混合级别
